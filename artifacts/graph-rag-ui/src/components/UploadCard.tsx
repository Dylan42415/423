...
export default function UploadCard({ icon, title, description, acceptedTypes }: UploadCardProps) {
  return (
    <Card className="bg-card border-card-border hover:border-primary/50 transition-all duration-200 cursor-pointer rounded-2xl group hover:shadow-[0_0_15px_rgba(59,130,246,0.1)]">
      ...
      <CardContent>
        {acceptedTypes && <p className="text-xs text-muted-foreground/70">{acceptedTypes}</p>}
      </CardContent>
    </Card>
  );
}
