...
const FILE_TYPE_TAGS = ["PDF", "DOCX", "TXT", "CSV", "JSON", "XML", "PNG", "JPG", "MP4", "MP3", "GeoJSON", "GraphML"];

export default function SettingsPage() {
  const [activeTab, setActiveTab] = useState("general");

  // General
  const [workspaceName, setWorkspaceName] = useState("Environmental Intelligence Hub");
  const [projectDesc, setProjectDesc] = useState("Multimodal intelligence platform for graph-based analysis and insight generation.");
    ...
    <div className="flex flex-col h-full">
      ...
      <div className="px-8 pt-8 pb-0 flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-1">Configure your workspace preferences and system controls.</p>
        </div>
        <div className="flex items-center gap-2">
          <Button variant="outline" size="sm" className="gap-1.5 text-muted-foreground" data-testid="button-settings-reset">
            <RotateCcw className="h-3.5 w-3.5" />
            Reset
          </Button>
          <Button size="sm" className="gap-1.5" data-testid="button-settings-save">
      ...
      <div className="flex-1 overflow-y-auto px-8 py-6 space-y-5">
        ...
        {activeTab === "general" && (
          <>
            <SettingsCard title="Workspace Settings" description="Basic configuration for your intelligence workspace.">
        ...
        {activeTab === "ai-query" && (
          <>
            <SettingsCard title="Query Preferences" description="Control how AI queries are processed and presented.">
              <div className="space-y-2">
                <p className="text-sm font-medium text-foreground">Default Query Mode</p>
                <p className="text-xs text-muted-foreground">Choose between natural language or direct Cypher queries.</p>
        ...
        {activeTab === "pipeline" && (
          <>
            <SettingsCard title="Ingestion Controls" description="Configure file processing and pipeline behavior.">
              <div className="space-y-1.5">
                <label className="text-sm font-medium text-foreground">Max Upload Size (MB)</label>
                <p className="text-xs text-muted-foreground">Maximum file size allowed per upload.</p>
                <Input
                  type="number"
                  value={maxUploadMB}
              ...
              <div className="space-y-2">
                <p className="text-sm font-medium text-foreground">Supported File Types</p>
                <p className="text-xs text-muted-foreground">File types accepted by the ingestion pipeline.</p>
        ...
        {activeTab === "graph" && (
          <>
            <SettingsCard title="Graph Visualization Settings" description="Configure how the knowledge graph is rendered and interacted with.">
              <DropdownField
        ...
        {activeTab === "notifications" && (
          <>
            ...
            <SettingsCard title="Delivery Channels" description="Where notifications are sent.">
              <ToggleSwitch
        ...
        {activeTab === "appearance" && (
          <>
            <SettingsCard title="Display Preferences" description="Customize the look and feel of the platform.">
              <ToggleSwitch
                label="Dark Mode"
              ...
              <div className="space-y-2 pt-1">
                ...
                <p className="text-xs text-muted-foreground">Primary highlight color used for active states and interactive elements.</p>
                <div className="flex items-center gap-3 mt-2">
                  {["#3b82f6", "#6366f1", "#8b5cf6", "#10b981", "#f59e0b", "#ef4444"].map((color) => (
                    <button
                      key={color}
