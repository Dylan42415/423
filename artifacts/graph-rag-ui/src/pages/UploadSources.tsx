...
import { uploadTypeConfigs } from "@/mock/mockUploadTypes";
  ...
  return (
    <div className="flex flex-col gap-8 w-full animate-in fade-in duration-500 pb-10">
      ...
      <div>
        ...
        {uploadTypeConfigs.length === 0 ? (
          <p className="text-sm text-muted-foreground">No data source types configured.</p>
        ) : (
        ...
        <Card className="bg-card border-card-border overflow-hidden rounded-2xl shadow-sm">
          <CardContent className="p-0">
            {uploadedFiles.length === 0 ? (
              <div className="flex items-center justify-center py-16 text-sm text-muted-foreground">No files uploaded yet.</div>
            ) : (
