import { graphStats, uploadedFiles, queryHistory, dashboardReports } from "@/mock/mockData";
  ...
  return (
    <div className="flex flex-col gap-6 w-full animate-in fade-in duration-500">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        <MetricCard title="Total Files"    value={graphStats.totalFiles}        icon={<FileText  className="h-4 w-4" />} change={graphStats.totalFilesChange}        changeType={graphStats.totalFilesChangeType} />
        <MetricCard title="Active Jobs"    value={graphStats.activeJobs}        icon={<Activity  className="h-4 w-4" />} change={graphStats.activeJobsChange}        changeType={graphStats.activeJobsChangeType} />
        <MetricCard title="Graph Entities" value={graphStats.entityCount}       icon={<Database  className="h-4 w-4" />} change={graphStats.entityCountChange}       changeType={graphStats.entityCountChangeType} />
        <MetricCard title="Relationships"  value={graphStats.relationshipCount} icon={<Network   className="h-4 w-4" />} change={graphStats.relationshipCountChange} changeType={graphStats.relationshipCountChangeType} />
      </div>
          ...
          <CardContent className="p-0 flex-1">
            <ScrollArea className="h-[300px]">
              {uploadedFiles.length === 0 ? (
                <div className="flex items-center justify-center h-full py-8 text-sm text-muted-foreground">No uploads yet.</div>
              ) : (
          ...
          <CardContent className="p-0 flex-1">
            <ScrollArea className="h-[300px]">
              {queryHistory.length === 0 ? (
                <div className="flex items-center justify-center h-full py-8 text-sm text-muted-foreground">No queries yet.</div>
              ) : (
                <div className="divide-y divide-border">
                  {queryHistory.slice(0, 5).map((query) => (
                    <div key={query.id} className="p-4 hover:bg-secondary/30 transition-colors flex flex-col gap-2">
                      ...
                      <p className={cn("text-sm text-foreground", query.mode === 'Cypher' && "font-mono text-xs text-primary")}>
                        {query.query}
                      </p>
                    </div>
                  ))}
        ...
        {dashboardReports.length === 0 ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            {dashboardReports.map((report) => (
              <Card key={report.id} className="bg-card border-card-border overflow-hidden rounded-xl group cursor-pointer hover:border-primary/50 transition-colors">
