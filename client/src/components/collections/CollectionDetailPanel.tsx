  ...
  return (
    <div className="flex flex-col h-full bg-card border-l border-border overflow-hidden animate-in slide-in-from-right duration-250">
      ...
      <div className="flex items-start justify-between gap-3 px-5 py-4 border-b border-border shrink-0">
        ...
        <button
          onClick={onClose}
          className="p-1.5 rounded-md text-muted-foreground hover:text-foreground hover:bg-accent/10 transition-colors shrink-0 mt-0.5"
          ...
          <div className="h-36 rounded-xl bg-background border border-border flex items-center justify-center relative overflow-hidden">
            ...
            <div className="relative z-10 text-center">
              ...
              <p className="text-[10px] text-muted-foreground">
                Graph preview — connect Neo4j renderer
              </p>
            </div>
          </div>
        </div>
              ...
              {collection.files.map((file) => (
                ...
                >
                  <Badge
                    variant="outline"
                    className={cn(
                      "text-[9px] font-mono shrink-0 border",
                  ...
                  <span className="text-xs text-foreground flex-1 truncate group-hover:text-primary transition-colors cursor-pointer">
                    {file.name}
                  </span>
                ...
                >
                  <span
                    className={cn(
                      ...
                      entityTypeColor[entity.type] || entityTypeColor.default,
                    )}
                  >
                    {entity.type}
                  </span>
              ...
              {collection.relatedQueries.map((q) => (
                ...
                >
                  <div className="flex items-center gap-2 mb-1.5">
                    <Badge
                      variant={q.mode === "Cypher" ? "default" : "secondary"}
                      className="text-[9px] h-4 px-1.5"
                    >
                      {q.mode === "Cypher" ? "Cypher" : "NL"}
                    </Badge>
                    <span className="text-[10px] text-muted-foreground flex items-center gap-0.5">
                      <Clock className="h-2.5 w-2.5" /> {q.runAt}
                    </span>
                  </div>
                  <p
                    className={cn(
