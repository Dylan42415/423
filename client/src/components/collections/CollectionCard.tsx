    ...
    >
      ...
      <div className="flex items-center gap-3 mb-3 text-[11px] text-muted-foreground">
        <span className="flex items-center gap-1">
          ...
          {collection.itemCount.toLocaleString()} files
        </span>
        <span className="text-border">·</span>
        <span className="flex items-center gap-1">
          <Network className="h-3 w-3 text-emerald-400" />
          {collection.entityCount.toLocaleString()} entities
        </span>
        <span className="text-border">·</span>
        <span className="flex items-center gap-1">
          <GitBranch className="h-3 w-3 text-violet-400" />
          {collection.relationshipCount.toLocaleString()} rels
        </span>
      </div>
      ...
      <div className="flex flex-wrap gap-1 mb-3">
        {collection.previewItems.map((item, idx) => (
          <span
            key={idx}
            className="text-[10px] bg-background border border-border rounded-md px-1.5 py-0.5 text-muted-foreground"
          >
            {item}
          </span>
        ))}
