...
const insightLineData: any[] = [];
const insightTrendItems: any[] = [];
const insightAnomalyData: any = { title: "No Anomalies", description: "System is healthy." };

type LineDataPoint = any;
type TrendItem = any;
type AnomalyData = any;
...
export default function InsightWidget({
              ...
              <div className="grid grid-cols-5 gap-1 mb-2">
                {Array.from({ length: 25 }).map((_, i) => (
                  <div
                    ...
                    style={{ opacity: (i % 7) / 8 + 0.15 }}
                  />
                ))}
              </div>
              <span className="text-xs text-muted-foreground">
                Activity Heatmap — connect data source
              </span>
            </div>
          </div>
        );
      ...
      case "cluster":
        return (
          <div className="h-[200px] w-full bg-secondary/30 rounded-lg flex items-center justify-center border border-border/50 overflow-hidden relative">
            ...
            <span className="text-xs text-muted-foreground z-10 bg-background/80 px-2 py-1 rounded">
              Entity Clusters — connect graph renderer
            </span>
          </div>
        );
  ...
  return (
    <Card className="bg-card border-card-border shadow-sm">
      ...
      <CardContent className="px-5 pb-5">{renderContent()}</CardContent>
    </Card>
  );
}
