  ...
  return (
    ...
    >
      <div className="h-16 flex items-center justify-between px-4 border-b border-sidebar-border">
        {expanded && (
          <span className="font-bold text-sidebar-foreground truncate tracking-tight text-lg">
            GraphRAG
          </span>
        )}
        <button
          onClick={() => setExpanded(!expanded)}
          className="p-1.5 rounded-md text-sidebar-foreground/70 hover:bg-sidebar-accent hover:text-sidebar-accent-foreground transition-colors ml-auto"
        ...
        {navItems.map((item) => {
          ...
          const content = (
            <Link
              ...
              data-testid={`link-sidebar-${item.label.toLowerCase().replace(/ & /g, "-").replace(/ /g, "-")}`}
            >
